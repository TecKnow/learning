...
def fastLongestPalindromes(seq):
    ...
    while i < seqLen:
        ...
        if i > palLen and seq[i - palLen - 1] == NucleotideComplement(seq[i]):
            palLen += 2
            i += 1
            continue
...
def NaivePalindrome(string):
    l = [0]*len(string)
    for x in range(len(string)-1):
        for y in range(0, int(len(string)/2)): # loop condition?
            if x-y < 0 or x+y+1 >= len(string):
                break
            elif not string[x-y] == NucleotideComplement(string[x+y+1]):
                break
            else: l[x-y]= max(l[x-y], 2*(y+1))
    return l

def VerifyReversePalindromes(DNAString, palindromes):
    for x in range(len(palindromes)):
        if palindromes[x] > 0 and not DNAString[x:x+palindromes[x]] == DNAStringReverseComplement(DNAString[x:x+palindromes[x]]):
            print("Not a reverse palindrome. Pos:", x, "String:", DNAString[x:x+palindromes[x]])

if __name__=="__main__":

    with open("RosREVPin.txt") as infile, open("RosREVPout.txt", mode="w") as outfile:
        print("Hello!")
        lines = infile.readlines()
        lines = [curLine.strip() for curLine in lines]
        string = ''.join(lines)
        resList = NaivePalindrome(string)
        for x in range(len(resList)):
            if resList[x] >= 4 and resList[x] <= 12:
                print(x+1, resList[x], file=outfile)
        VerifyReversePalindromes(string, resList)
